...
-- develop individual option views
CREATE VIEW cruise_control AS 
	SELECT sales.id, options.option AS cruise_control 
		FROM sales 
	JOIN options 
...
CREATE VIEW auto_reverse_system AS 
	select sales.id, options.option AS auto_reverse_system
		FROM sales
	JOIN options
...
CREATE VIEW pedestrian_alerting_sound AS
	SELECT sales.id, options.option AS pedestrian_alerting_sound
		FROM sales
	JOIN options
		ON sales."PedestrianAlertingSoundint" = options.id;
SELECT * FROM pedestrian_alerting_sound WHERE pedestrian_alerting_sound = 'Standard';


CREATE VIEW blind_spot_monitor AS
	SELECT sales.id, options.option AS blind_spot_monitor
		FROM sales
	JOIN options
		ON sales."BlindSpotMon" = options.id;
SELECT * FROM blind_spot_monitor WHERE blind_spot_monitor !='';


CREATE VIEW forward_collision_warning AS 
	SELECT sales.id, options.option AS forward_collision_warning
		FROM sales
	JOIN options
		ON sales."ForwardCollisionWarning" = options.id;
SELECT * FROM forward_collision_warning WHERE forward_collision_warning != '';


CREATE VIEW keyless_ignition AS
	SELECT sales.id, options.option AS keyless_ignition
		FROM sales
	JOIN options
		ON sales."KeylessIgnition" = options.id;
SELECT * FROM keyless_ignition WHERE keyless_ignition != '';


CREATE VIEW pedestrian_auto_emergency_brake AS
	SELECT sales.id, options.option AS pedestrian_auto_emergency_brake
		FROM sales
	JOIN options
		ON sales."PedAutoEBrake" = options.id;
SELECT * FROM pedestrian_auto_emergency_brake WHERE pedestrian_auto_emergency_brake != '' ;


-- create complete options view
CREATE VIEW complete_options_view AS
	SELECT cruise_control.id, cruise_control, auto_reverse_system, pedestrian_alerting_sound,
		blind_spot_monitor, forward_collision_warning, keyless_ignition, pedestrian_auto_emergency_brake
		FROM cruise_control
	JOIN auto_reverse_system
		ON cruise_control.id = auto_reverse_system.id
	JOIN pedestrian_alerting_sound
		ON cruise_control.id = pedestrian_alerting_sound.id
	JOIN blind_spot_monitor
		ON cruise_control.id = blind_spot_monitor.id
	JOIN forward_collision_warning
		ON cruise_control.id = forward_collision_warning.id
	JOIN keyless_ignition
		ON cruise_control.id = keyless_ignition.id
	JOIN pedestrian_auto_emergency_brake
		ON cruise_control.id = pedestrian_auto_emergency_brake.id;
		
SELECT * FROM complete_options_view;

CREATE VIEW all_sales_view AS
	SELECT 
		sales.id, 
		...
		cruise_control, auto_reverse_system, pedestrian_alerting_sound,
			blind_spot_monitor, forward_collision_warning, keyless_ignition, pedestrian_auto_emergency_brake
	...
	JOIN complete_options_view
		ON sales.id = complete_options_view.id;
